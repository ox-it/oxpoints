...
<xsl:key name="Lib_All" match="tei:place[@type='library']" use="1"/>
<xsl:key name="place_All" match="tei:place" use="1"/>
...
<xsl:key name="Occupies"
	 match="tei:relation[@name='occupies']"
	 use="substring-after(@active,'#')"/>
<xsl:key name="Controls"
	 match="tei:relation[@name='controls']"
	 use="substring-after(@passive,'#')"/>
